...
function apiToBoardDeal(d: ApiDestination, origin: string): BoardDeal {
  ...
  return {
    ...
    price,
    priceFormatted: price != null ? `$${price}` : 'Check', // "from" prefix is added by the card UI, not here (board rows need raw price for split-flap)
    ...
    typicalPriceLow: d.typicalPriceLow,
    typicalPriceHigh: d.typicalPriceHigh,
